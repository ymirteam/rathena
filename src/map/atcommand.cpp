...
ACMD_FUNC(who) {
	...
	for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter))	{
		if (!((pc_has_permission(pl_sd, PC_PERM_HIDE_SESSION) || pc_isinvisible(pl_sd)) && pc_get_group_level(pl_sd) > level) && !pl_sd->state.autotrade) { // you can look only lower or same level. [Start]: Not count auto trade
	...
	for (uint16 k = 0; k < count; k++) {
		...
		if( mob->get_bosstype() == BOSSTYPE_MVP ){
			...
			for (i = 0; i < MAX_MVP_DROP_TOTAL; i++) {
				...
				mvppercent = mob_getdroprate(&sd->bl, mob, mob->mvpitem[i].rate, drop_modifier); // [Start]
				if(battle_config.item_drop_mvp_mode == 0) {
					mvpremain -= mvppercent;
				}
				if (mvppercent > 0) {
					j++;
					// [Start] Divide by 100 was custom
					if (j == 1) {
						if (id->slots)
							sprintf(atcmd_output2, " %s[%d]  %02.02f%%", item_db.create_item_link(id->nameid).c_str(), id->slots, mvppercent / 100);
						else
							sprintf(atcmd_output2, " %s  %02.02f%%", item_db.create_item_link(id->nameid).c_str(), mvppercent / 100);
					}
					else {
						if (id->slots)
							sprintf(atcmd_output2, " - %s[%d]  %02.02f%%", item_db.create_item_link(id->nameid).c_str(), id->slots, mvppercent / 100);
